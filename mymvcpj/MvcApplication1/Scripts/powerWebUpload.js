...
(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        var webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            server: applicationPath + '/WebUploader/Process',
        ...
        if (fileDataStr && opts.PostbackHold) {
            ...
            $.each(jsonData.fileList, function (index, fileData) {
              ...
              (opts.ShowDownload ? '<div class="download"></div>' : '') +
               (opts.ShowDelBtn ? '<div class="del"></div>' : '') +
            '</div>');
            });
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            if (opts.uploaderinput != '' && opts.fileNumLimit==1) { //文件上传值回显，用于保存到数据库 中
                var mfname = response.filePath.replace(opts.savefileroute, "");
        ...
        $list.on("click", ".download", function () {
            ...
            $.each(jsonData.fileList, function (index, item) {
                if (item && item.queueId === id) {
                    downloadfile = item;
                    $("#" + opts.hiddenInputId).val(JSON.stringify(jsonData));
                    $.post(applicationPath + '/WebUploader/GetPathId', { 'filePathName': downloadfile.filePath, 'fileName': downloadfile.name }, function (pathid) {
                        window.location.href = applicationPath + '/WebUploader/Download?pathid=' + pathid;
                    });
                    return;
                }
            });
    ...
    $.fn.powerWebUpload = function (options) {
        ...
        $(ele).each(function (idnex, item) {
            ...
            if (typeof PowerJs != 'undefined') {
                PowerJs.Core.lazyLoad(applicationPath + "/Scripts/webuploader/webuploader.css", function () { }, 'css');
                PowerJs.Core.lazyLoad(applicationPath + "/Scripts/webuploader/webuploader.min.js", function () {
